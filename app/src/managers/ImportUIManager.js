...
export const ImportUI = {
    ...
    updateMassImportPreview() {
        ...
        if (columnCount === 1 && lines.length > 1) {
            ...
            warningEl.innerHTML = `<i class="fas fa-exclamation-triangle"></i> Une seule colonne détectée. Vérifiez le séparateur.`;
        ...
        if (DOM.importGenerateBtn) {
            if (isProcessing) {
                ...
                if (DOM.importGenerateBtn.dataset.originalContent) {
                    ...
                    DOM.importGenerateBtn.innerHTML = `<i class="fas fa-bolt" aria-hidden="true"></i> Générer`;
                }
            }
        }
    }
